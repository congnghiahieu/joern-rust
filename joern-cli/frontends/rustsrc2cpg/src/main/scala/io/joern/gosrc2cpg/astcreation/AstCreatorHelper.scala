...
import io.joern.rustsrc2cpg.ast.{GoModule, Token}
...
trait AstCreatorHelper(implicit withSchemaValidation: ValidationMode) {
  ...
  def getPackageFullname(goModule: GoModule, packageFilePath: String, packageIdentifier: Identifier): String = {
    val relativePath = Paths.get(goModule.modulePath).relativize(Paths.get(packageFilePath).getParent).toString
  ...
  def generateNodeFromFunctionType(
    fileName: String,
    functionType: FunctionType
  ): (Seq[NewMethodParameterIn], NewMethodReturn) = {
    val functionTypeIdentifier = getFunctionReturnType(functionType)
    val returnNode             = methodReturnNode(functionTypeIdentifier, functionTypeIdentifier.typeFullName)
...
//                    val parameterType = astForExpression(fileName, field.typeExpression.get)
          ...
          .map(field => {
            ...
            expression match {
              ...
              case starExpression: StarExpression =>
                starExpression.expression match {
                  case Some(expr) =>
                    expr match {
                      case identifier: Identifier => identifier.typeFullName
                      case selectorExpression: SelectorExpression =>
                        getTypeNameFromSelectorExpression(selectorExpression)
                      case _ =>
              ...
              case arrayType: ArrayType =>
                astForExpression("", arrayType).root match {
              ...
              case mapType: MapType =>
                astForExpression("", mapType).root match {
  ...
  def getTypeNameFromSelectorExpression(selectorExpression: SelectorExpression): String = {
    if (
      selectorExpression.expression.isDefined
