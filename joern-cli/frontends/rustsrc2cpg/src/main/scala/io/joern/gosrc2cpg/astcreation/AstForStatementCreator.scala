...
import io.joern.rustsrc2cpg.Constant
import io.joern.rustsrc2cpg.ast.Token
import io.joern.x2cpg.{Ast, Defines, ValidationMode}
import io.joern.rustsrc2cpg.ast.nodes.*
...
trait AstForStatementCreator(implicit withSchemaValidation: ValidationMode) {
  ...
  private def astForCaseClause(fileName: String, caseClause: CaseClause): Seq[Ast] = {
    ...
    if (caseClause.list.nonEmpty) {
      caseClause.list.foreach(expr => {
        val node    = jumpTargetNode(caseClause, "case", expr.code)
  ...
  private def astForIfStatement(fileName: String, ifStatement: IfStatement): Ast = {
    ...
    val ifNode = controlStructureNode(ifStatement, ControlStructureTypes.IF, ifStatement.code)
    ...
    controlStructureAst(ifNode, Some(conditionAst), Seq(bodyAst, elseAst) ++ initAst)
  }
  ...
  private def astForSwitchStatement(fileName: String, switchStatement: SwitchStatement): Ast = {
    ...
    controlStructureAst(switchNode, Option(conditionAst), switchBodyAst)
  }
  ...
  private def astForTypeSwitchStatement(fileName: String, typeSwitchStatement: TypeSwitchStatement): Ast = {
        ...
        case Some(node) =>
          node match {
            case identifier: NewIdentifier =>
              Ast(
                identifierNode(
                  typeSwitchStatement.assign.get,
                  identifier.name,
                  identifier.code,
            ...
            case local: NewLocal =>
              val identNode = identifierNode(typeSwitchStatement.assign.get, local.name, local.code, local.typeFullName)
    ...
    val call =
      callNode(typeSwitchStatement, typeSwitchStatement.code, Operators.is, Operators.is, DispatchTypes.STATIC_DISPATCH)
    val conditionAst = callAst(call, identifierAsts)

    controlStructureAst(switchNode, Option(conditionAst), switchBodyAsts)
  }
  ...
  private def astForBranchStatement(fileName: String, branchStatement: BranchStatement): Ast = {
    ...
    Ast(controlStructureNode(branchStatement, controlStructureType, branchStatement.code))
  }
          ...
          case identifier: Identifier =>
            if (values.length > index) {
              ...
              if (value.isInstanceOf[FunctionLiteral]) {
                asts.addOne(astForExpression(fileName, value.asInstanceOf[FunctionLiteral]))
              } else {
//                                val typeFullName = getTypeFullNameFromExpression(value)
                usedPrimitiveTypes.add(identifier.typeFullName)
                val local = localNode(identifier, identifier.name.get, identifier.code, identifier.typeFullName)
                ...
                val call =
                  callNode(value, value.code, Operators.assignment, Operators.assignment, DispatchTypes.STATIC_DISPATCH)
                val leftAst  = astForExpression(fileName, identifier)
                val rightAst = astForExpression(fileName, value)
                asts.addOne(callAst(call, Seq(leftAst, rightAst)))
              }
              index += 1
            }
          case _ => {}
      ...
      val leftExprAst  = assignmentStatement.lhs.map(expr => astForExpression(fileName, expr))
      val rightExprAst = assignmentStatement.rhs.map(expr => astForExpression(fileName, expr))
      ...
      asts.addOne(callAst(call, leftExprAst.toSeq ++: rightExprAst.toSeq))
    }
    asts.toSeq
  }
