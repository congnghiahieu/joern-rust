...
trait AstForSpecificationCreator(implicit schemaValidationMode: ValidationMode) { this: AstCreator =>
  ...
  def astForSpecification(fileName: String, parentFullName: String, specification: Specification): Seq[Ast] = {
    specification match {
      case importSpecification: ImportSpecification => Seq(astForImportSpecification(fileName, importSpecification))
  ...
  private def astForImportSpecification(fileName: String, importSpecification: ImportSpecification): Ast = {
    ...
    val importNode = newImportNode(importSpecification.code, importPath, importAs, importSpecification.path.get)
    Ast(importNode)
  }
...
//                        )
    ...
    typeSpecification.typeExpression match {
      case Some(typeExpression) =>
        typeExpression match {
          case structType: StructType => Seq(astForStructType(filename, parentFullName, typeSpecification, structType))
  ...
  private def astForStructType(
    ...
    val typeDecl = typeDeclNode(typeSpecification, name, fullname, filename, typeSpecification.code)
    ...
    val constructor = createStructConstructor(filename, typeSpecification, structType)

    Ast(typeDecl)
      .withChild(modifierAst)
      .withChild(constructor)
      .withChildren(fieldAsts.map(fieldAst => {
  ...
  private def createStructConstructor(
    ...
    val methodNode_ = methodNode(
      structType,
      typeSpecification.name.get.name.get + ".<init>",
    ...
    val blockNode_ = blockNode(typeSpecification.name.get, typeSpecification.name.get.code, "void")

    methodAst(
      methodNode_,
      Seq(),
      Ast(blockNode_),
      methodReturnNode(structType, typeSpecification.fullName),
      Seq(modifier)
    )
  }
