...
import io.joern.rustsrc2cpg.Constant.PrimitiveTypes
import io.joern.rustsrc2cpg.ast.Token
import io.joern.x2cpg.{Ast, Defines, ValidationMode}
import io.joern.rustsrc2cpg.ast.nodes.{Identifier, MapType, SelectorExpression, *}
...
trait AstForExpressionCreator(implicit validationMode: ValidationMode) {
  ...
  def astForExpression(fileName: String, expression: Expression): Ast = {
    expression match {
      case indexExpression: IndexExpression               => astForIndexExpression(fileName, indexExpression)
      case starExpression: StarExpression                 => astForStarExpression(fileName, starExpression)
      case typeAssertExpression: TypeAssertExpression     => astForTypeAssertExpression(fileName, typeAssertExpression)
      case unaryExpression: UnaryExpression               => astForUnaryExpression(fileName, unaryExpression)
      case binaryExpression: BinaryExpression             => astForBinaryExpression(fileName, binaryExpression)
      case functionLiteral: FunctionLiteral               => astForFunctionLiteral(fileName, functionLiteral)
      case callExpression: CallExpression                 => astForCallExpression(fileName, callExpression)
      case compositeLiteral: CompositeLiteral             => astForCompositeLiteral(fileName, compositeLiteral)
      case basicLiteralExpression: BasicLiteralExpression => astForBasicLiteral(fileName, basicLiteralExpression)
      case identifier: Identifier                         => astForIdentifier(fileName, identifier)
      case parenthesizedExpression: ParenthesizedExpression =>
        parenthesizedExpression.expression match {
          case Some(expr) => astForExpression(fileName, expr)
          case None       => Ast()
        }
      case selectorExpression: SelectorExpression => astForSelectorExpression(fileName, selectorExpression)
      case keyValueExpression: KeyValueExpression => astForExpression(fileName, keyValueExpression.value.get)
...
//            case ellipsisExpression: EllipsisExpression
  ...
  private def astForInterfaceType(fileName: String, interfaceType: InterfaceType): Ast = {
    ...
    val typeDecl = typeDeclNode(interfaceType, interfaceType.code, anonymousInterfaceName, fileName, interfaceType.code)
...
//        }
    ...
    Ast(typeRefNode(interfaceType, interfaceType.code, anonymousInterfaceName))
  }
  ...
  private def astForSliceExpression(fileName: String, sliceExpression: SliceExpression): Ast = {
    ...
    val call =
      callNode(sliceExpression, sliceExpression.code, operator, operator, DispatchTypes.STATIC_DISPATCH, None, None)
  ...
  private def astForMapType(fileName: String, mapType: MapType): Ast = {
    ...
    Ast(newIdentifierNode(typeName, typeName))
  }
...
//        }
  ...
  private def astForSelectorExpression(fileName: String, selectorExpression: SelectorExpression): Ast = {
    ...
    val callNode = newOperatorCallNode(Operators.fieldAccess, selectorExpression.code, Some(fieldTypeFullName))
  ...
  private def astForUnaryExpression(fileName: String, unaryExpression: UnaryExpression): Ast = {
    ...
    val call = callNode(unaryExpression, unaryExpression.code, operator, operator, DispatchTypes.STATIC_DISPATCH)
  ...
  private def astForBinaryExpression(fileName: String, binaryExpression: BinaryExpression): Ast = {
    if (
      binaryExpression.code.equals("len(any.TypeUrl) == len(name) || any.TypeUrl[len(any.TypeUrl)-len(name)-1] == '/'")
    ) {
      println("Breakpoint")
    }
    ...
    val call =
      callNode(binaryExpression, binaryExpression.code, operator, operator, DispatchTypes.STATIC_DISPATCH, None, None)
  ...
  private def astForFunctionLiteral(fileName: String, functionLiteral: FunctionLiteral): Ast = {
    ...
    val method = methodNode(functionLiteral, fullname, functionLiteral.code, fullname, Option(fullname), fileName)
    scope.pushNewScope(method)
    val (parameters, returnNode) = generateNodeFromFunctionType(fileName, functionLiteral.functionType.get)
    val bodyAst                  = astForStatement(fileName, functionLiteral.body.get)
    val methodAst_ = methodAst(method, parameters.map(parameter => Ast(parameter)), bodyAst.head, returnNode)
    scope.popScope()
    ...
    val funcDecl =
      typeDeclNode(functionLiteral, fullname, fullname, fileName, functionLiteral.code, parentType, parentFullname)
  ...
  private def preReqForCallNode(
    ...
    function match
      case identifier: Identifier =>
        ...
        (identifier.name.get, callExpression.functionSignature, callExpression.functionFullName, typeFullName, Seq())
      ...
      case parenthesizedExpression: ParenthesizedExpression =>
        parenthesizedExpression.expression match {
          case Some(expr) =>
            expr match {
              case identifier: Identifier =>
                (Operators.cast, Operators.cast, Operators.cast, identifier.typeFullName, Seq())
              case _ => ("", "", "", "", Seq())
            }
          case None => ("", "", "", "", Seq())
        }
      case arrayType: ArrayType =>
        (arrayType.code + "()", arrayType.code + "()", arrayType.code + "()", arrayType.code, Seq())
...
//    }
  ...
  private def astForBasicLiteral(fileName: String, basicLiteral: BasicLiteralExpression): Ast = {
    ...
    val literal = literalNode(basicLiteral, basicLiteral.code, typeName)
    Ast(literal)
  }
    ...
    identifier.name match {
      case Some(name) =>
        if (identifier.name.get.equals("true") || identifier.name.get.equals("false")) {
          val literal = literalNode(identifier, identifier.code, PrimitiveTypes.BOOLEAN)
          Ast(literal)
        } else if (!name.equals("_")) {
          scope.lookupVariable(name) match {
            case Some((localNode, typeFullname)) =>
              val identNode = identifierNode(identifier, name, identifier.code, typeFullname)
              Ast(identNode).withRefEdge(identNode, localNode)
            case None =>
              if (identifier.typeFullName != "") {
                Ast(newIdentifierNode(identifier.name.getOrElse(""), identifier.typeFullName))
              } else {
                Ast()
              }
          }
